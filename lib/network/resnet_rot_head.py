...
class RotHeadNet(nn.Module):
    def __init__(self, in_channels, num_layers=3, num_filters=256, kernel_size=3, output_kernel_size=1,
        ...
        nn.ConvTranspose2d(in_channels, num_filters, kernel_size=kernel_size, stride=2, padding=padding,
    ...
    def forward(self, x, x_f64=None, x_f32=None, x_f16=None):
            if self.freeze:
                with torch.no_grad():
                    ...
                    return conf.detach(), coor_x.detach(), coor_y.detach(), coor_z.detach()
            else:
                for i, l in enumerate(self.features):
                    x = l(x)
                conf = x[:, :2, :, :] 
